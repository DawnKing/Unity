...
namespace GameProtocol.LeagueMessage
{
    ...
    /// 通知客户端，联赛报名结果
    public sealed class LeagueEnrollResult : IProtocol
    {
        public const short MsgType = MessageType.MSG_LEAGUE_ENROLL_RESULT;
        ...
        public static void Send(uint teaid, uint session, uint zone_id, uint result, object className)
        {
            var packet = new LeagueEnrollResult
            {
                teaid = teaid,
                session = session,
                zone_id = zone_id,
